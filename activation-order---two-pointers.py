...
class Solution:
    def maxTotal(self, value: List[int], limit: List[int]) -> int:
        limit_value = [(lim, val) for lim, val in zip(limit, value)]
        limit_value.sort(key=lambda pair: (pair[0], -pair[1]))
        cur_active = 0
        total_act = 0
        i = j = 0
        while i < len(limit_value):
            lim, val = limit_value[i]
            if cur_active < lim:
                total_act += val
                
                while j < len(limit_value) and limit_value[j][0] <= cur_active:
                    j += 1
                    
                cur_active = max(0, i - j + 1)
            i = max(i + 1, j)
